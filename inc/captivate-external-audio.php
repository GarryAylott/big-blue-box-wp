...
if ( ! function_exists( 'bbb_get_captivate_audio_url' ) ) {
	function bbb_get_captivate_audio_url( $guid ) {
		...
		if ( get_option( 'disable_captivate_api' ) ) {
			bbb_log( '🛑 API disabled via admin setting.' );
		...
		if ( ! defined( 'CAPTIVATE_USER_ID' ) || ! defined( 'CAPTIVATE_API_TOKEN' ) ) {
			bbb_log( '❌ Captivate credentials are missing.' );
			return false;
		}
		...
		if ( $cached ) {
			bbb_log( '✅ Returning cached audio URL.' );
			return $cached;
		}
		...
		if ( ! is_admin() && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) && ! ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ) {
			if ( $rate_limited ) {
				bbb_log( '⏳ Rate limit hit for front-end API fetch.' );
				return false;
			}
		...
		if ( $auth_code < 200 || $auth_code >= 300 ) {
			bbb_log( '❌ Captivate auth failed. HTTP status: ' . $auth_code );
			return false;
		}
		...
		if ( ! $bearer_token ) {
			...
			bbb_log( '❌ Captivate auth failed. Response body (truncated): ' . $snippet );
			return false;
		}

		bbb_log( '✅ Got bearer token.' );
		...
		if ( is_wp_error( $episode_response ) ) {
			bbb_log( '❌ Episode request failed: ' . $episode_response->get_error_message() );
			return false;
		}

		$episode_code = wp_remote_retrieve_response_code( $episode_response );
		if ( $episode_code < 200 || $episode_code >= 300 ) {
			bbb_log( '❌ Episode request failed. HTTP status: ' . $episode_code );
			return false;
		}
		...
		if ( ! is_array( $episode_data ) ) {
			bbb_log( '❌ Episode response invalid JSON.' );
			return false;
		}
		...
		if ( ! $media_id ) {
			...
			bbb_log( '❌ No media_id found in episode. Will retry next time. Body (truncated): ' . $snippet );
			return false;
		}

		bbb_log( "✅ Got media_id: $media_id" );
		...
		if ( is_wp_error( $media_response ) ) {
			bbb_log( '❌ Media request failed: ' . $media_response->get_error_message() );
			return false;
		}

		$media_code = wp_remote_retrieve_response_code( $media_response );
		if ( $media_code < 200 || $media_code >= 300 ) {
			bbb_log( '❌ Media request failed. HTTP status: ' . $media_code );
			return false;
		}
		...
		if ( ! is_array( $media_data ) ) {
			bbb_log( '❌ Media response invalid JSON.' );
			return false;
		}
		...
		if ( $audio_url ) {
			...
			bbb_log( '✅ Audio URL found and cached.' );
			return $audio_url;
		}

		$snippet = substr( $media_body, 0, 200 );
		bbb_log( '❌ No media_url found in media response. Body (truncated): ' . $snippet );
		return false;
	}
}
